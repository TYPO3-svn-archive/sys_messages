		...
		*/
		function todos_finalizeWorkflow($workflowRecord, $relRecord) {
			...
			if ($relRecord['tablename'] == $table && $TCA[$table]) {
				...
				if (is_array($itemRecord) && $this->loadExecutor()) {
					if('-1' == $itemRecord['pid']) {
...
#						list($target_pid) = explode(',', $workflowRecord['final_target']);
						$this->wfExe->publishNewRecord($table,$uid,$workflowRecord);
					}					 
					return true;
				} else {
		...
		*/
		function todos_beginWorkflow($workflowRecord,$relRecord) {
			global $TCA;
...
#			list($working_area_pid) = explode(',', $workflowRecord['working_area']);
			...
			case 'new inside':
				if('pages'==$relRecord['tablename']) {
					return $this->wfExe->createNewRecord($relRecord['tablename'],$relRecord['idref']);
				} else {
					die('Error in: '.__FILE__.','.__LINE__.debug_backtrace());
				}
				break;
			case 'new after':
				return $this->wfExe->createNewRecord($relRecord['tablename'],-1*$relRecord['idref']);
				break;
			case 'version':
				return $this->wfExe->createNewVersionOfRecord($relRecord['tablename'], $relRecord['idref']);
				break;
			case 'delete':
				break;
			case 'move':
						...
						if ($data['sys_todos_users_mm'][$key]['status']['code'] > 0) {
							...
							switch($field_values['status']) {
								...
								if ($this->loadExecutor() && is_array($workflowRecord) && $workflowRecord['tablename'] && !$row['rec_reference']) {
									$recId = $this->todos_beginWorkflow($workflowRecord,$row);
									if ($recId) {
										$field_values['rec_reference'] = $recId;
										$RD_URL = $this->getEditRedirectUrlForReference($recId,('new'==substr(trim($row['action']), 0, 3)));
									} else {
								...
								case 103:
									// Finalize
									//TODO: check for rights to publish, not whether user has created it.
								if ($this->loadDefinition() && $this->wfDef->getWorkFlow($workflowRecord['uid'],$row['table'],$row['action'],'publish')) {
									// Must own
									$field_values['uid_foreign'] = $row['cruser_id'];
...
// 				}
			...
			if ($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res)) {
				...
				if ($row['type'] && $row['type'] != 'plain') {
					...
					if (substr($wF, 0, 3) == "wf_" && $this->loadDefinition()) {
						...
						if (is_array($workflowDef) && $workflowDef['tablename']) {
							$formA[] = array($LANG->getLL('todos_workflowDescr').":&nbsp;", $workflowDef['description']);
						}
					}
				}
				...
				if (!t3lib_div::_GP('editTodo')) {
					...
					while ($rel_row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res)) {
						$msg[] = '<HR>'.$this->todos_printStatus($rel_row, $row, $workflowDef, $tUid, $GLOBALS['TYPO3_DB']->sql_num_rows($res));
					}
		...
		*/
		function todos_printStatus($rel_row, $todo_row, $workflow_row, $tUid, $countOfInstances = 0) {
			...
			$statusLabels[6] = htmlspecialchars($LANG->getLL('todos_status_review'));
...
// 					$prevUsersGroups[] = $logDat['uid_foreign_before'];
			...
			if ($rel_row['rec_reference']) {
				...
				//TODO: href skal ikke være direkte til db-modulet, men et
				//link tilbage til taskcenter, med parametre der gør at
				//sys_todo åbner redigeringen i en IFRAME, nap kode herfra til
				//visning af ikon, mv i create formen
				if ($recordRelated) {
			...
			//TODO: check disse rettigheder op mod den nye model. Tilføj action, send to publishing.

			// Unset certain items which we do not present in the menu under certain circumstances.
// 			if (!is_array($workflow_row) || !$rel_row['tablename'] || $rel_row['rec_reference']) unset($statusLabels_copy[2]);
			if (!is_array($workflow_row) || !$rel_row['tablename'] || $rel_row['rec_reference'] ||
					!($this->loadDefinition() && $this->wfDef->getWorkflow($workflow_row['uid'],$rel_row['tablename'],$rel_row['action'],'edit')))	
				unset($statusLabels_copy[2]);

 			if (!count($revUsers) || (!$rel_row['rec_reference'] && is_array($workflow_row)) ||
 					!($this->loadDefinition() && $this->wfDef->getWorkflow($workflow_row['uid'],$rel_row['tablename'],$rel_row['action'],'edit')))	
				unset($statusLabels_copy[3]);


			if ($rel_row['state']=='reviewed' || !($this->loadDefinition() && $this->wfDef->getWorkflow($workflow_row['uid'],$rel_row['tablename'],$rel_row['action'],'review')))	unset($statusLabels_copy[6]);

// 			if (!is_array($workflow_row) || !$rel_row['tablename'] || !$rel_row['rec_reference']) unset($statusLabels_copy[103]);
			if ($rel_row['state']!='reviewed' || !($this->loadDefinition() && $this->wfDef->getWorkflow($workflow_row['uid'],$rel_row['tablename'],$rel_row['action'],'publish')))	unset($statusLabels_copy[103]);
...
// 					}
		...
		* @todo move to def-class
		*/
		function todos_getPublishingUsers($wfUid) {
			$res = $GLOBALS['TYPO3_DB']->exec_SELECT_mm_query(
			'be_users.uid,be_users.username,be_users.realName',
				'sys_workflows',
				...
				if (!is_array($editRec)) {
					...
					if ($wF == 'plain') {
						...
						if (is_array($workflowDef) && t3lib_div::_GET('table')) {
							$formA[] = array($LANG->getLL('todos_type').":&nbsp;", $workflowDef['title']);
							// Get groupnames for todo-tasks
							...
							$formA[] = array('Action:',t3lib_div::_GET('action').' - '.t3lib_iconWorks::getIconImage(t3lib_div::_GET('table'),$refRecord,$this->backPath).$refRecord['title'].
												 '<input type="hidden" name="data[sys_todos]['.$type.'][action]" value="'.t3lib_div::_GET('action').'">'.
		...
		function blindUserNames($users,$allowedGroups) {
			foreach($allowedGroups as $group) {
				foreach($users as $key => $user) {
					if(t3lib_div::inList($user['usergroup_cached_list'],$group)) {
						$ok[$key] = $user;
						unset($users[$key]);
					}
				}
