	...
	function main()	{
		...
		//		debug($POST);
				...
				if ($line)	{
					...
					$recData["sys_template"][$tplRow[uid]][$field] = $tplRow[$field].$line;
		...
		//	debug($tmpl->tsbrowser_depthKeys);
			...
			if ($POST["search"] && $POST["search_field"])	{		// If any POST-vars are send, update the condition array
				$tmpl->tsbrowser_depthKeys=$tmpl->ext_getSearchKeys($theSetup, "",	$POST["search_field"], array());
		//		debug($tmpl->tsbrowser_depthKeys);
		...
		//		$this->pObj->MOD_MENU["ts_browser_toplevel_".$bType]["-"]="---";
				$this->pObj->MOD_MENU["ts_browser_toplevel_".$bType] = $this->pObj->MOD_MENU["ts_browser_toplevel_".$bType] + $this->pObj->MOD_SETTINGS["ts_browser_TLKeys_".$bType];
			}
		
		//	debug($this->pObj->MOD_MENU["ts_browser_toplevel"]);
