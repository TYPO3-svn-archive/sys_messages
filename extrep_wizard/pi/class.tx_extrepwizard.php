...
require_once(t3lib_extMgm::extPath("extrep_wizard")."pi/class.tx_extrepwizard_compilefiles.php");

class tx_extrepwizard extends tx_extrepwizard_compilefiles {
	var $varPrefix = "tx_extrep";		// redundant from "extrep"
	var $siteBackPath = "";
	var $EMmode=0;	// If run from Extension Manager, set to 1.
	...
	function initWizArray()	{
		...
		$inArray=unserialize(base64_decode($this->piData["wizArray_ser"]));
		$this->wizArray = is_array($inArray) ? $inArray : array();
		if (is_array($this->piData["wizArray_upd"]))	{
			$this->wizArray = t3lib_div::array_merge_recursive_overrule($this->wizArray,$this->piData["wizArray_upd"]);
		}
	...
	function mgm_wizard()	{
		...
		if ($this->piData["viewResult"])	{
			...
			if ($saveKey)	{
				...
				$this->pObj->importExtFromRep(0,$this->piData["loc"],0,$uploadArray);
			} else $content = $this->fw("<strong>Error:</strong> Please enter an extension key first!<BR><BR>");
		} elseif ($this->piData["totalForm"])	{
			$content = $this->totalForm();
		} elseif ($this->piData["downloadAsFile"])	{
			if ($saveKey)	{
				$this->makeFilesArray($this->saveKey);
				$uploadArray = $this->makeUploadArray($this->saveKey,$this->fileArray);
			...
			$action = explode(":",$this->piData["wizAction"]);
		...
		if ((string)$this->piData["wizSubCmd"])	{
			if ($wasContent)	$content.='<input name="update2" type="submit" value="Update..."> ';
		}
		$content.='
					<input type="hidden" name="'.$this->piFieldName("wizAction").'" value="'.$this->piData["wizAction"].'">
					<input type="hidden" name="'.$this->piFieldName("wizSubCmd").'" value="'.$this->piData["wizSubCmd"].'">
					'.$this->cmdHiddenField().'
				</td>
			</tr>
			</form>
	...
	function getFormContent()	{
		switch((string)$this->piData["wizSubCmd"])	{
			case "tables":
		...
		while(list($k,$v)=each($this->options))	{
			...
			if (is_array($items))	{
				...
				while(list($k2,$conf)=each($items))	{
					$this->piData["wizSubCmd"]=$k;
					$this->piData["wizAction"]="edit:".$k2;
	...
	function sidemenu()	{
#debug($this->piData);
		$actionType = $this->piData["wizSubCmd"].":".$this->piData["wizAction"];
...
# onClick="setFormAnchorPoint(\'_top\')"
	...
	function view_result()	{
		...
		$content.= $this->fw("<BR><strong>Author name:</strong> ".$this->userField("name")."
							<BR><strong>Author email:</strong> ".$this->userField("email"));
	...
	function add_cat_emconf()	{
		...
		if ($action[0]=="edit")	{
			...
			$subContent="<strong>Author Name:</strong><BR>".
				$this->renderStringBox($ffPrefix."[author]",$piConf["author"]?$piConf["author"]:$this->userField("name"));
			$lines[]='<tr'.$this->bgCol(3).'><td>'.$this->fw($subContent).'</td></tr>';

				// Author/Email
			$subContent="<strong>Author email:</strong><BR>".
				$this->renderStringBox($ffPrefix."[author_email]",$piConf["author_email"]?$piConf["author_email"]:$this->userField("email"));
			$lines[]='<tr'.$this->bgCol(3).'><td>'.$this->fw($subContent).'</td></tr>';
		}
...
//			$lines[]='<tr'.$this->bgCol(2).'><td>'.$this->fw($v[1]).'</td></tr>';
	...
	function presetBox(&$piConfFields)	{
		$_PRESETS = $this->piData["_PRESET"];

		$optValues = array();
		$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery('*', 'tx_extrepwizard_static_presets', '');
		while($presetRow = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res))	{
			...
			// Session presets:
		$ses_optValues=array();
		$sesdat = $GLOBALS["BE_USER"]->getSessionData("tx_extrepwizard");
		...
		while(list($k,$v)=each($fConf))	{
			if ($v["type"] && trim($v["fieldname"]))	{
				...
				if (t3lib_div::_GP($this->varPrefix.'_CMD_'.$v["fieldname"].'_SAVE_x'))	{
					...
					$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery('fieldname', 'tx_extrepwizard_static_presets', 'fieldname="'.$GLOBALS['TYPO3_DB']->quoteStr($v['fieldname'], 'tx_extrepwizard_static_presets').'"');
					if ($GLOBALS['TYPO3_DB']->sql_num_rows($res) || $v["_DELETE"])	{
						if ($v["_DELETE"])	{
							$GLOBALS['TYPO3_DB']->exec_DELETEquery('tx_extrepwizard_static_presets', 'fieldname="'.$GLOBALS['TYPO3_DB']->quoteStr($v['fieldname'], 'tx_extrepwizard_static_presets').'"');
						} else {
							$GLOBALS['TYPO3_DB']->exec_UPDATEquery('tx_extrepwizard_static_presets', 'fieldname="'.$GLOBALS['TYPO3_DB']->quoteStr($v['fieldname'], 'tx_extrepwizard_static_presets').'"', $datArr);
						}
					} else {
						$GLOBALS['TYPO3_DB']->exec_INSERTquery("tx_extrepwizard_static_presets", $datArr);
					}
				}
			} else {
...
#		debug($newFConf);
		...
		$GLOBALS["BE_USER"]->setAndSaveSessionData("tx_extrepwizard",$sesdat);
...
//				$fDetails.=$this->renderCheckBox($prefix."[conf_files_mm]",$fConf["conf_files_mm"])."DB relations (very rare choice, normally the commalist is fine enough)<BR>";
	...
	function path_resources($subdir="res/")	{
		return substr(t3lib_extMgm::extPath("extrep_wizard"),strlen(PATH_site))."pi/".$subdir;
	}
...
//			}
	...
	function fw($str)	{
		return '<span style="font-family:verdana,arial,sans-serif; font-size:11px;">'.$str.'</span>';
	}
	function userField($field)		{
		$dat = $GLOBALS["TSFE"]->fe_user->user[$field];
		return $dat;
	}
...
// Include extension?
if (defined("TYPO3_MODE") && $TYPO3_CONF_VARS[TYPO3_MODE]["XCLASS"]["ext/extrep_wizard/pi/class.tx_extrepwizard.php"])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]["XCLASS"]["ext/extrep_wizard/pi/class.tx_extrepwizard.php"]);
}
