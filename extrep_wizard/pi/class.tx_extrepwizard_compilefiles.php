...
class tx_extrepwizard_compilefiles {
	...
	function makeRepositoryUpdateArray($extKey)	{
		$extKeyRec = $this->getExtKeyRecord($extKey);
		$this->extKey = $extKey;

		if (is_array($extKeyRec))	{
			
			$dataArr = $this->renderVersion("","");
			$dataArr["pid"] = $this->dbPageId;
	
			$dataArr["backend_title"]=$extKey.", KS ".date("H:i d-m-y");
	
			$dataArr["extension_key"]=$extKey;
			$dataArr["extension_uid"]=$extKeyRec["uid"];
			$dataArr["last_upload_by_user"]=$GLOBALS["TSFE"]->fe_user->user["uid"];
			$dataArr["upload_comment"]="Written by KickStarter ".date("H:i d-m-y");
			$dataArr["last_upload_referer"]="";
			$dataArr["last_upload_returnUrl"]="";
			$dataArr["last_upload_date"]=time();
	
			$file = t3lib_extMgm::extPath("extrep_wizard")."res/notfound.gif";
			$dataArr["icondata"]=serialize(array(
				"name" => "ext_icon.gif",
				"size" => filesize($file),
				"mtime" => filemtime($file),
				"is_executable" => (TYPO3_OS=='WIN' ? 0 : is_executable($file)),
				"content" => t3lib_div::getUrl($file),
				"content_md5" => md5(t3lib_div::getUrl($file))
			));

			$dataArr["emconf_title"] = trim($extKeyRec["title"]);			
			$dataArr["emconf_description"] = trim($extKeyRec["description"]);
			$dataArr["emconf_category"] = "example";
			$dataArr["emconf_state"] = "";
			$dataArr["emconf_author"] = $this->userField_ext("name");
			$dataArr["emconf_author_email"] = $this->userField_ext("email");
			$dataArr["emconf_author_company"] =  $this->userField_ext("company");

			$this->makeFilesArray($extKey);
			$dataArr = array_merge($dataArr,$this->makeEMCONFpreset("emconf_"));
#debug($dataArr);
			
//			dependencies,priority,module,modify_tables

			if (is_array($this->fileArray))	{
				reset($this->fileArray);
				$filesA=array();
				while(list($k,$d)=each($this->fileArray))	{
					unset($d["content"]);
					$d["filename"]=$k;
					$filesA[$k]=$d;
				}
				$dataArr["files"] = serialize($filesA);
				$rawDat = serialize($this->fileArray);
			...
			$existRec = $this->getLatestRepVersion($extKeyRec["uid"]);
			if (is_array($existRec) && !t3lib_div::inList("stable,alpha,beta",$existRec["emconf_state"]))	{
				...
				debug("UPDATED: ".$extKeyRec["extension_key"].", uid=".$existRec["uid"]);
			} else {
				$this->cObj->DBgetInsert("tx_extrep_repository", $this->dbPageId, $dataArr, implode(",",array_keys($dataArr)), TRUE);
				$extRepId = $GLOBALS['TYPO3_DB']->sql_insert_id();
				debug("INSERTED: ".$extKeyRec["extension_key"].", uid=".$extRepId);
			}
			
#			debug($dataArr);
#			debug($extKey);
#			debug($this->wizArray);
		} else debug($extKey." not found!!");
	}
...
#debug(array($this->fileArray["ext_typoscript_setup.txt"]));
		...
		$this->addFileToFileArray("doc/wizard_form.dat",serialize($this->wizArray));
			...
			// icon:
		$this->addFileToFileArray("ext_icon.gif",t3lib_div::getUrl(t3lib_extMgm::extPath("extrep_wizard")."res/notfound.gif"));
...
//str_replace("\\'","'",addslashes($this->getSplitLabels($config,"title")))		
		...
		',2);
				...
				// Add wizard icon
			$this->addFileToFileArray($pathSuffix."icon_".$tableName.".gif",t3lib_div::getUrl(t3lib_extMgm::extPath("extrep_wizard")."res/".$config["defIcon"]));
		...
		switch($t)	{
			...
			case "textarea_nowrap":
				...
				if ($fConf["conf_wiz_example"])	{
					...
					$this->addFileToFileArray($id."/wizard_icon.gif",t3lib_div::getUrl(t3lib_extMgm::extPath("extrep_wizard")."res/notfound.gif"));
				...
				for($a=0;$a<t3lib_div::intInRange($fConf["conf_select_items"],1,20);$a++)	{
					...
					if ($fConf["conf_select_icons"] && $t=="select")	{
						$icon = ', t3lib_extMgm::extRelPath("'.$extKey.'")."'."selicon_".$id."_".$a.".gif".'"';
										// Add wizard icon
						$this->addFileToFileArray("selicon_".$id."_".$a.".gif",t3lib_div::getUrl(t3lib_extMgm::extPath("extrep_wizard")."res/wiz.gif"));
					} else $icon="";
...
#debug($config);
		...
		if ($config["plus_wiz"] && $config["addType"]=="list_type")	{
			...
			$this->addFileToFileArray($pathSuffix."class.".$cN."_wizicon.php",$this->PHPclassFile($extKey,$pathSuffix."class.".$cN."_wizicon.php",$indexContent,"Class that adds the wizard icon."));
				...
				// Add wizard icon
			$this->addFileToFileArray($pathSuffix."ce_wiz.gif",t3lib_div::getUrl(t3lib_extMgm::extPath("extrep_wizard")."res/wiz.gif"));

				// Add clear.gif
			$this->addFileToFileArray($pathSuffix."clear.gif",t3lib_div::getUrl(t3lib_extMgm::extPath("extrep_wizard")."res/clear.gif"));
...
*/		
		...
		$this->addFileToFileArray($pathSuffix."clear.gif",t3lib_div::getUrl(t3lib_extMgm::extPath("extrep_wizard")."res/clear.gif"));
			...
			// Add clear.gif
		$this->addFileToFileArray($pathSuffix."moduleicon.gif",t3lib_div::getUrl(t3lib_extMgm::extPath("extrep_wizard")."res/notfound_module.gif"));
	...
	function renderExtPart_cm($k,$config,$extKey)	{
		...
		$this->addLocalConf($this->ext_locallang,$config,"title","cm",$k);
			...
			// Add icon
		$this->addFileToFileArray($pathSuffix."cm_icon.gif",t3lib_div::getUrl(t3lib_extMgm::extPath("extrep_wizard")."res/notfound_module.gif"));
		...
		if ($config["second_level"])	{
			...
			$this->addLocalConf($this->ext_locallang,array("title_activate"=>"...Second level ->"),"title_activate","cm",$k,0,1);
				// Add activate icon
			$this->addFileToFileArray($pathSuffix."cm_icon_activate.gif",t3lib_div::getUrl(t3lib_extMgm::extPath("extrep_wizard")."res/notfound_module.gif"));
		}
...
#		$this->printPre($content);
	...
	function PHPclassFile($extKey,$filename,$content,$desrc,$SOBE_class="",$SOBE_extras="")	{
		$file = trim($this->sPS('	
			...
			*  Copyright notice
			*  
			*  (c) 2004 '.$this->userField_ext("name").' ('.$this->userField_ext("email").')
			*  All rights reserved
			*
			*  This script is part of the TYPO3 project. The TYPO3 project is 
			*  free software; you can redistribute it and/or modify
			...
			***************************************************************/
			 ...
			 * '.$desrc.'
			 *
			 * @author	'.$this->userField_ext("name").' <'.$this->userField_ext("email").'>
			 */
		'));
	...
	function userField_ext($k)	{
		$v = $this->userField($k);
#debug($v,1);
#debug($k,1);
		if (!$v && $k=="name")	$v = $this->wizArray["emconf"][1]["author"];
		if (!$v && $k=="email")	$v = $this->wizArray["emconf"][1]["author_email"];
		return $v;
#debug($v,1);
	}
...
// Include extension?
if (defined("TYPO3_MODE") && $TYPO3_CONF_VARS[TYPO3_MODE]["XCLASS"]["ext/extrep_wizard/pi/class.tx_extrepwizard_compilefiles.php"])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]["XCLASS"]["ext/extrep_wizard/pi/class.tx_extrepwizard_compilefiles.php"]);
}
