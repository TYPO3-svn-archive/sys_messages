...
class tx_tstemplateceditor extends t3lib_extobjbase {
  ...
  //JULLE: this function does nothing but since it doesn't return an array as expected, and as done by the original function in extobjbase, it breaks the template module in php5
  // 	function modMenu()	{
  // 		global $LANG;
	...
	function main()	{
		...
		//	debug($GLOBALS["HTTP_POST_FILES"]);
			if (t3lib_div::_POST("submit"))	{
				...
				$tmpl->changed=0;
				$tmpl->ext_procesInput(t3lib_div::_POST(),$GLOBALS["HTTP_POST_FILES"],$theConstants,$tplRow);
		//		debug($tmpl->changed);
		//		debug($tmpl->raw);
		//		$tmpl->changed=0;
				if ($tmpl->changed)	{
					...
					$recData=array();
					$recData["sys_template"][$tplRow[uid]]["constants"] = implode($tmpl->raw,chr(10));
