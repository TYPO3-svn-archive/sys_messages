	...
	function main()	{
		...
		';
			...
			$headerSection = $this->doc->getHeader("pages",$pageinfo,$pageinfo["_thePath"]).'<br>'.
				...
				'<span title="'.htmlspecialchars($pageinfo['_thePathFull']).'">'.htmlspecialchars(t3lib_div::fixed_lgd_pre($pageinfo['_thePath'],50)).'</span>';
				...
				if (t3lib_div::testInt($k))	{
					if (isset($pArray[$k."_"]))	{
						$lines[]='<tr class="bgColor4">
							...
							t3lib_iconWorks::getIconImage("pages",t3lib_BEfunc::getRecord("pages",$k),$GLOBALS["BACK_PATH"],' align="top" title="ID: '.$k.'"').
							t3lib_div::fixed_lgd($pArray[$k],30).'</a></td>
						...
						$lines[]='<tr>
							...
							t3lib_iconWorks::getIconImage("pages",t3lib_BEfunc::getRecord("pages",$k),$GLOBALS["BACK_PATH"]," align=top").
							t3lib_div::fixed_lgd($pArray[$k],30).'</td>
							</tr>';
					}
