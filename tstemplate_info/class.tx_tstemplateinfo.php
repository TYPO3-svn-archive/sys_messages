				...
				if (is_array($POST["data"]))	{
					while(list($field,$val)=each($POST["data"]))	{
						switch($field)	{
							...
							case "sitetitle":
							case "description":
								$recData["sys_template"][$tplRow[uid]][$field] = $val;
							break;
							case "resources":
								$tmp_upload_name = t3lib_div::upload_to_tempfile($GLOBALS["HTTP_POST_FILES"]["resources"]["tmp_name"]);	// If there is an uploaded file, move it for the sake of safe_mode.
								if ($tmp_upload_name)	{
									if ($tmp_upload_name!="none" && $GLOBALS["HTTP_POST_FILES"]["resources"]["name"])	{
										$alternativeFileName[$tmp_upload_name] = trim($GLOBALS["HTTP_POST_FILES"]["resources"]["name"]);
